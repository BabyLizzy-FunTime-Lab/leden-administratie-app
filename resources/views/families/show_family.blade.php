<x-layout>
    <x-headerWrapper>
        ...
        @else
            ...
            <p class="lead"> Familie ID: {{ $family->id }}</p>
            <p class="lead"> Familie Address: {{ $family->address }}</p>
            <a class="btn btn-primary header-wrapper_action" href="#addNewFamilyMember" role="button">Familie Lid Toevoegen</a>
            <a class="btn btn-primary header-wrapper_action" href="#editFamily" role="button">Familie info Aanpassen</a>
        @endif
    </x-headerWrapper>
    <x-contentWrapper>
        ...
        @else
            <ul class="list-group">
                @foreach($family->users as $user)
                    <x-listItem>
                        <a class="list-item_link" href="{{ route('family_member.show', $user->id) }}">
                            <h5>{{ $user->name }}</h5>
                        </a>
                        <a class="list-item_action" href="{{ route('family_member.show', $user->id) }}">Bekijken</a>
                    </x-listItem>
                @endforeach
            </ul>
        @endif
    ...
    <x-contentWrapper>
        ...
        <div class="table-container">
            <table class="table table-hover">
                ...
                @endphp
                <tr>
                    <td class="table_colspan-totaal" colspan="2"></td>
